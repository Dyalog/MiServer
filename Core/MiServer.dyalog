...
⍝ ↓↓↓--- Begin MildServer Core Code
    ...
    ∇ r←RunServer arg;RESUME;⎕TRAP;Common;cmd;name;port;wres;ref;nspc;sink;Stop;secure;certpath;flags;z;rc;rate;ErrorCount;idletime;msg;uid;ts
      ...
      :While ~Stop
          ...
          ⍝ ⎕←100↑,⍕(⎕SIZE'wres')':'wres
          ⍝ ↑↑↑ comment out to keep an eye on whassup
    ...
    ∇ r←AllocatePort;ipv;server;maxsize
    ⍝ Starts Conga, allocates TCP/IP port
      ipv←'Protocol'Config.IPVersion
      maxsize←100000 ⍝ Maximum block size
      {}#.DRC.SetProp'.' 'ReadyStrategy' 1 ⍝ Round Robin (should be in config file)
      :If Config.Secure
          ...
          :AndIf ⊃∧/#.Files.Exists¨Config.(CertFile KeyFile)
              ...
              →(0≠1⊃r←#.DRC.Srv'' ''Config.Port'Raw'maxsize('X509'server)('SSLValidation'Config.SSLFlags)ipv)⍴0 ⍝ Must have Conga v2.1
          ...
          :EndIf
      :Else
          →(0≠1⊃r←#.DRC.Srv'' ''Config.Port'Raw'maxsize ipv)⍴0
      :EndIf
