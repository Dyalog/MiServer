:Class index : MiPageSample
    ...
    ∇ js←APLtoJS vtv;act;sel;dummy;data;tv;fr;to
...
⍝    ∇
    ...
    ∇ r←onSelectSample;code;control;depth;folder;html;i;iframe;node;p;page;sample;samples;section;simple;sp;space;t;tab;text;titles;url;file;title;spacedir;spctrl;files;dir;pathfile;ctrlsec;pagea;outfile;tie;js;desc;item;out;link
      :Access Public
     ⍝ When a sample is selected, call this
      ...
      :If depth=2  ⍝ Element
          ...
          r,←'#SampleDesc'Replace'The below samples demonstrate usage of ',spctrl,'.'
          ...
          out←New _.div
        ⍝  'width' 'height' 'border' out.Set'800px' '400px' '2px inset'
          'style'out.Set'width:800px;height:400px;border:2px inset'
          :For file :In files[;1]
              pathfile←dir,'/',file
              code←{0::,⊂'[file read failed]' ⋄ #.UnicodeFile.ReadNestedText ⍵}pathfile
              ...
              :If (⊂spctrl)∊1↓¨(' '∘=⊂⊢)' ',ctrlsec ⍝ Split Space-delimited list
                  ...
                  link←New _.a ctrlsec                ⍝ link with element names
                  'target' 'href'link.Set'_blank'url ⍝ new tab
                  js←'#SampleTitle'Replace link       ⍝ Generate JS
     
                  js,←'#SampleDesc'Replace desc
                  iframe←'src' 'width' 'height'(New _.iframe).Set(url,'?NoWrapper=1')800 400
                  js,←'#SampleFrame'Replace iframe
                  js,←'#SampleSource'Replace #.HTMLInput.APLToHTMLColour code
                  js←APLtoJS js
                  item.On'click' 0 ''js
              :EndIf
          :EndFor
