  ...
  ⍝  NodeID←{⍺←⊢ ⋄ ('#node',⊃⍵)∘,¨⍕¨⍺+⍳⍴⊃⌽⍵} ⍝ Generate node ids (⍵='L' items) optional (⍺=offset)
    ...
    ∇ PopulateMid mid;url;code;frame;title;navbuts
     ...
     ⍝ Read framed pages
      url←⊃CURRFILES
      code←Dread url
     ⍝ Add a menu off action buttons
      navbuts←⊂'List related' 'List samples using the same control or samples in the same category'
      navbuts,←⊂'More info' 'Get reference information about the current control'
      navbuts,←⊂'Pop out' 'Open the current sample in a new tab or window'
      navbuts,←⊂'View source' 'View source code for the current sample'
      {('#to',⊃⌽Words⊃⍵)'.navbut'('title'{⍺ ⍵}⊃⌽⍵)mid.Add _.Button(⊃⍵)}¨⌽navbuts ⍝ reversed because of "float: right"
      mid.Add _.Handler'.navbut' 'click' 'OnNavBut'
     ...
     ⍝ Create and fill placeholder for embedded page
      ...
      mid.Add _.hr
     ⍝ Button to go from sample source to top of page
      '#source' '.navbut' 'title="Return to the sample output"'mid.Add _.Button'Top of page'
     ⍝ Create placeholder for source code
      '#SampleSource' '.samplesource'mid.Add _.div
    ...
    ∇ r←GenJS(page title desc iframe code)
     ⍝ Generate JavaScript for filling placeholders
      ...
      r,←(×≢title)/'#SampleTitle'Replace title
      r,←'#SampleDesc'Replace desc
      r,←'#SampleFrame'Replace iframe
      r,←'#SampleSource'Replace''⍝(×≢code)/'small;border:none'#.HTMLInput.APLToHTMLColour code
    ∇
